...
# ========= SOZLAMALAR =========
DATABASE_URL = "postgresql://user:password@localhost:5432/kinobot"  # o'zingiznikini yozing
...
class MandatoryChannel(Base):
    ...
    id = Column(Integer, primary_key=True)
    username = Column(String(64), unique=True, nullable=False)   # faqat username (@ belgisiz)

    def __repr__(self):
        return f"<MandatoryChannel(username={self.username})>"
...
class Database:
    ...
    # ===== MOVIE CRUDs =====
    def add_movie(self, code, name, category, description, file_id=None, file_type="video", parts=None) -> bool:
        try:
    ...
    def add_serial(self, code, name, category, description, parts) -> bool:
        return self.add_movie(code, name, category, description, parts=parts, file_type="serial")
    ...
    # ===== MAJBURIY KANALLAR DB CRUDs =====
    def add_channel(self, username: str) -> bool:
        session = self.get_session()
        try:
            username = username.lower()
            exists = session.query(MandatoryChannel).filter_by(username=username).first()
            if exists:
                return False
            ch = MandatoryChannel(username=username)
            session.add(ch)
            session.commit()
            logger.info(f"Majburiy kanal qo‘shildi: @{username}")
            return True
        except Exception as e:
            logger.error(f"Channel qo‘shishda xato: {e}")
            session.rollback()
            return False
        finally:
            session.close()
    ...
    def remove_channel(self, username: str) -> bool:
        ...
        try:
            username = username.lower()
            ch = session.query(MandatoryChannel).filter_by(username=username).first()
            if not ch:
                return False
            session.delete(ch)
            session.commit()
            logger.info(f"Majburiy kanal o‘chirildi: @{username}")
            return True
        except Exception as e:
            logger.error(f"Channel o‘chirishda xato: {e}")
            session.rollback()
            return False
        finally:
            session.close()
    ...
    def list_channels(self) -> list:
        session = self.get_session()
        try:
            channels = session.query(MandatoryChannel).all()
            return [ch.username for ch in channels]
        except Exception as e:
            logger.error(f"Channels ro‘yxati xatosi: {e}")
            return []
        finally:
            session.close()
